...
test('notification styles use stable window widths', () => {
  ...
  assert.equal(calculateNotificationWidth('top-right', 'island'), 380)
  assert.equal(calculateNotificationWidth('top-center', 'island'), 380)
  assert.equal(calculateNotificationMaxHeight('island'), 120)
  ...
  assert.equal(normalizeNotificationStyle('island'), 'island')
