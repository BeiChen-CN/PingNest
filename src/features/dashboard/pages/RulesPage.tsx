  ...
  return <><button type="button" className="side-editor-mask" aria-label="关闭规则编辑器" onClick={() => { if (!saving) onCancel() }} />
    <aside className="side-editor" role="dialog" aria-modal="true" aria-label={rule ? '编辑静音规则' : '新增静音规则'}>
      <form className="side-editor-form" onSubmit={(event) => { event.preventDefault(); void submit() }}>
        ...
        <div className="side-editor-actions"><button type="submit" className="md-button filled" disabled={!canSave || saving}>{saving ? '正在保存' : '保存'}</button><button type="button" className="md-button outlined" disabled={saving} onClick={onCancel}>取消</button></div>
      </form>
    </aside></>
}
...
export function RulesPage({ config, entries, saveConfig }: { config: AppConfig; entries: NotifyCenterEntry[]; saveConfig: SaveConfig }) {
  ...
  return <section className="rules-page page-body">
    <section className="surface scope-panel">
      <div className="scope-panel-head">
        <div><b>通知范围</b><small>先决定哪些会话可以触发桌面通知</small></div>
        <SelectField label="通知范围" value={config.notificationFilterMode} options={[{ value: 'all', label: '所有会话' }, { value: 'whitelist', label: '仅选中会话' }, { value: 'blacklist', label: '除选中会话外' }]} onChange={(value) => void saveConfig('notificationFilterMode', value as AppConfig['notificationFilterMode'])} />
      </div>
      {config.notificationFilterMode !== 'all' && <SessionPicker sessions={sessions} selected={config.notificationFilterList} onToggle={toggleFilterSession} />}
    </section>
    <div className="rules-actions">
      <div><span>规则列表</span><small>{rules.filter((rule) => rule.enabled).length} / {rules.length} 条启用 · 按关键词或会话匹配</small></div>
      <button className="md-button filled" onClick={() => setEditor(null)}><Plus size={15} />新增规则</button>
    </div>
    <div className="rules-layout">
      <section className="surface rule-table">
        <div className="table-row header"><span>规则</span><span>条件</span><span>匹配</span><span>状态</span><span>范围</span><span /></div>
        {rules.length ? rules.map((rule) => (
          <div className={'table-row' + (rule.enabled ? '' : ' muted')} key={rule.id}>
            <b>{rule.name || '未命名规则'}</b>
            <span>{rule.keywords.join('、') || '指定会话'}</span>
            <span>{rule.matchMode === 'all' ? '全部' : '任一'}</span>
            <Switch checked={rule.enabled} onChange={(enabled) => void saveRule({ ...rule, enabled })} label={`切换${rule.name || '规则'}`} />
            <span>{rule.sessionIds.length ? `${rule.sessionIds.length} 个会话` : '全部会话'}</span>
            <div className="table-actions">
              <button className="icon-button" onClick={() => setEditor(rule)} aria-label={`编辑${rule.name || '规则'}`} title="编辑"><Pencil size={15} /></button>
              <button className="icon-button danger-icon" onClick={() => setDeleteRule(rule)} aria-label={`删除${rule.name || '规则'}`} title="删除"><Trash2 size={15} /></button>
            </div>
          </div>
        )) : (
          <div className="empty-state"><VolumeX size={24} /><b>暂无静音规则</b><button className="md-button filled" onClick={() => setEditor(null)}><Plus size={14} />新增规则</button></div>
        )}
      </section>
      {editor !== false && <RuleEditor key={editor?.id || 'new'} rule={editor} sessions={sessions} onSave={saveRule} onCancel={() => setEditor(false)} />}
    </div>
