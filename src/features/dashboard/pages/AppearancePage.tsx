  ...
  return <section className="appearance-page page-body">
    <div className="appearance-redesign">
      ...
      <section className="surface appearance-card appearance-behavior-card">
        ...
        <div className="appearance-click-setting"><label htmlFor="click-action">点击通知后</label><SelectField id="click-action" label="点击通知后" value={config.notificationClickBehavior} options={[{ value: 'open-wechat', label: '激活微信窗口' }, { value: 'open-app', label: '在通知历史中查看' }, { value: 'none', label: '无操作' }]} onChange={(value) => void saveConfig('notificationClickBehavior', value as AppConfig['notificationClickBehavior'])} /><small className="appearance-click-hint">当前版本会激活微信主窗口，不会模拟点击或强制定位到未公开的内部会话。</small></div>
      </section>
    </div>
  </section>
}
